...
- **奖状边界模式（Award Boundaries）** — 在网格地图上叠加 DXCC / WAPC / WAJA / WAZ / WAS 国界·省界·分区边界，已确认区域绿色填充；中国边界按官方主张绘制（含藏南、喀喇昆仑走廊），香港/澳门作为独立实体显示（DXCC 321/152），港澳标签强制显示
- **LoTW / Wavelog 网格同步** — 直连 ARRL Logbook（账号密码验证）或自托管 Wavelog 下载确认报告，worked / roamed 网格自动更新（ADIF 字段顺序无关的健壮解析）；同步失败按凭据错误/限流/超时分类提示
